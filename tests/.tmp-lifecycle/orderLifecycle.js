...
export const LEGACY_ORDER_STATUSES = ['paid', 'refunded', 'cancelled'];
...
export const TERMINAL_ORDER_STATUSES = ['refunded'];
...
export function canTransition(from, to) {
    ...
    if (source === to)
        return true; // idempotent re-apply is always safe
...
export function isPendingDeliveryPayload(payload) {
    if (!payload)
        return true;
...
export function resolveOrderLifecycle(order) {
    ...
    const payment = pick(order.payment_status, PAYMENT_STATUSES, base.payment);
    const fulfillment = pick(order.fulfillment_status, FULFILLMENT_STATUSES, base.fulfillment);
    const delivery = pick(order.delivery_status, DELIVERY_STATUSES, base.delivery);
    const hasRealPayload = !isPendingDeliveryPayload(order.delivered_items) ||
    ...
    const legacyFulfilled = status === 'paid' && order.fulfillment_status == null && order.delivery_status == null;
...
export const ORDER_STATUS_SHORT_LABELS = {
    pending: '🕒 Pending',
 ...
 */
export function sanitizeSupplierStatus(raw) {
    if (!raw)
        return null;
...
export function supplierStatusLine(order) {
...
export function adminTransitionsFor(current) {
 ...
 */
export function lifecyclePatchFor(to, from) {
    const d = DERIVED[to];
